...
for i = trialNum:length(testStims_img)
  ...
  if phaseCfg.isExp && phaseCfg.secUntilBlinkBreak > 0 && (GetSecs - blinkTimerStart) >= phaseCfg.secUntilBlinkBreak && i > 3 && i < (length(testStims_img) - 3)
    thisGetSecs = GetSecs;
...
%       keyIsDown_new = false;
  ...
  if expParam.useNS
    ...
    if ~isnan(newRespPromptOn)
      % new response prompt
      [NSEventStatus, NSEventError] = et_NetStation('Event', 'PROM', newRespPromptOn, .001,...
        'subn', expParam.subject, 'sess', sesName, 'phas', phaseName, 'pcou', int32(phaseCount),...
    ...
    if ~isnan(recallRespPromptOn)
      ...
      [NSEventStatus, NSEventError] = et_NetStation('Event', 'PROM', recallRespPromptOn, .001,...
        'subn', expParam.subject, 'sess', sesName, 'phas', phaseName, 'pcou', int32(phaseCount),...
