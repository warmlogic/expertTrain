...
message = 'Starting judgment phase...';
if expParam.useNS
  % start recording
  ...
  message = 'Starting data acquisition for judgment phase...';
...
for i = 1:length(phaseCfg.instruct.expo)
  WaitSecs(1.000);
  et_showTextInstruct(w,phaseCfg.instruct.expo(i),cfg.keys.instructContKey,...
...
for i = trialNum:length(expoStims_img)
  ...
  if expParam.useNS && phaseCfg.isExp && i > 1 && i < length(expoStims_img) && mod((i - 1),phaseCfg.impedanceAfter_nTrials) == 0
    % run the impedance break
    thisGetSecs = GetSecs;
    fprintf(logFile,'%f\t%s\t%s\t%s\t%d\t%d\t%s\n',thisGetSecs,expParam.subject,sesName,phaseName,phaseCount,phaseCfg.isExp,'IMPEDANCE_START');
    fprintf(phLFile,'%f\t%s\t%s\t%s\t%d\t%d\t%s\n',thisGetSecs,expParam.subject,sesName,phaseName,phaseCount,phaseCfg.isExp,'IMPEDANCE_START');
  ...
  if phaseCfg.isExp && cfg.stim.secUntilBlinkBreak > 0 && (GetSecs - blinkTimerStart) >= cfg.stim.secUntilBlinkBreak && i > 3 && i < (length(expoStims_img) - 3)
    thisGetSecs = GetSecs;
