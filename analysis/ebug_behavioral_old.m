...
procDir = '/Network/Servers/curran-lab.colorado.edu/Volumes/RAID/curranlab/Data/EBUG/Behavioral/Sessions';
subjects = {
%     'EBUG001';
%     'EBUG002';
%     'EBUG003';
...
if collapsePhases
  behfile = 'EBUG_behav_results_collapsed.mat';
else
  behfile = 'EBUG_behav_results.mat';
end

load(fullfile(getenv('HOME'),'data','EBUG','Behavioral','Sessions',behfile));
...
anovaFile = fullfile(getenv('HOME'),'data','EBUG','Behavioral','Sessions','ANOVA',sprintf('EBUG_ANOVA_prepost_%s.txt',measure));
fid = fopen(anovaFile,'w+');

thisHeader = [];
for i = 1:length(sesNames)
  thisHeader = cat(2,thisHeader,sprintf('%s%s',sesNames{i},repmat(sprintf('\t'),1,length(trainConds) * length(famLevel))));
end
fprintf(fid,'\t%s\n',thisHeader);

thisHeader = [];
for i = 1:length(trainConds)
  thisHeader = cat(2,thisHeader,sprintf('%s%s',trainConds{i},repmat(sprintf('\t'),1,length(famLevel))));
end
...
anovaFile = fullfile(getenv('HOME'),'data','EBUG','Behavioral','Sessions','ANOVA',sprintf('EBUG_ANOVA_train_%s.txt',measure));
fid = fopen(anovaFile,'w+');
