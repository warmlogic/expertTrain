...
%   end
  ...
  for sesNum = 1:length(expParam.sesTypes)
    ...
    if exist(fullfile(saveDir,subjects{sub},sessions{sesNum}),'dir')
      if ~isfield(events,expParam.sesTypes{sesNum})
      ...
      fprintf('Data directory for %s %s %s does not exist. Not processing.\n',subjects{sub},sessions{sesNum},expParam.sesTypes{sesNum});
    end
  end
  %else
