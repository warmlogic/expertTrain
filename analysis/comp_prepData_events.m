function comp_prepData_events(subjects,prep_eeg)
% comp_prepData_events(subjects,prep_eeg)
%
% Purpose
    ...
    for sesNum = 1:length(expParam.sesTypes)
      ...
      for pha = 1:length(expParam.session.(sesName).phases)
        ...
        if cfg.stim.(sesName).(phaseName)(phaseCount).isExp
          ...
          % create the events
          events = comp_createEvents(events,dataroot,subjects{sub},sesNum,sesName,phaseName,phaseCount);
...
%     continue
  ...
  saveEvents(eventsOutfile_sub,'events');
  fprintf('Done.\n');
  ...
  if prep_eeg == 1
    for sesNum = 1:length(expParam.sesTypes)
      ...
      %
      % TODO: create/fix comp_events2ns so it doesn't rely on events in
      % session directory
      fprintf('comp_events2ns IS NOT DONE YET!!!\n');
      comp_events2ns(dataroot,subjects{sub},sesNum);
