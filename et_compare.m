...
for i = trialNum:length(vStimTex)
  ...
  fprintf(logFile,'%f\t%s\t%s\t%s\t%d\t%d\t%s\t%d\t%d\t%s\t%s\t%d\t%d\t%d\n',...
    imgOn,...
    expParam.subject,...
    sesName,...
    phaseName,...
  ...
  % Write stimulus presentation to file:
  fprintf(phLFile,'%f\t%s\t%s\t%s\t%d\t%d\t%s\t%d\t%d\t%s\t%s\t%d\t%d\t%d\n',...
    imgOn,...
    expParam.subject,...
    sesName,...
    phaseName,...
  ...
  if expParam.useNS
    ...
    [NSEventStatus, NSEventError] = et_NetStation('Event', 'FIXT', preStimFixOn, .001,...
      ...
      'expt',phaseCfg.isExp,...
      'bloc', int32(b),...
      'trln', int32(i), 'stmn', stimName, 'famn', fNum, 'spcn', specNum,...
...
for i = trialNum:length(wiStim2Tex)
  ...
  if any(wiStim2(i).familyNum == famNumSubord)
    isSubord = true;
    specNum1 = int32(wiStim1(i).speciesNum);
    specNum2 = int32(wiStim2(i).speciesNum);
  elseif any(wiStim2(i).familyNum == famNumBasic)
    isSubord = false;
    specNum1 = int32(0);
    specNum2 = int32(0);
  end
  ...
  % ISI between trials
  if phaseCfg.comp_isi > 0
    WaitSecs(phaseCfg.comp_isi);
  end
  ...
  WaitSecs(phaseCfg.comp_preStim1(1) + ((phaseCfg.comp_preStim1(2) - phaseCfg.comp_preStim1(1)).*rand(1,1)));
  
  % draw the stimulus
  Screen('DrawTexture', w, wiStim1Tex(i), [], stimImgRect);
  % and fixation on top of it
  Screen('TextSize', w, cfg.text.fixSize);
  DrawFormattedText(w,cfg.text.fixSymbol,'center','center',cfg.text.fixationColor, cfg.text.instructCharWidth);
  ...
  if cfg.text.printTrialInfo
    fprintf('Trial %d of %d: stim1 (%s): family %d (%s), species %d (%s), exemplar %d (%d). Same (1) or diff (0): %d.\n',i,length(wiStim2Tex),wiStim1(i).fileName,wiStim1(i).familyNum,wiStim1(i).familyStr,wiStim1(i).speciesNum,wiStim1(i).speciesStr,wiStim1(i).exemplarNum,wiStim1(i).exemplarName,wiStim1(i).same);
  end
  
  % while loop to show stimulus until "duration" seconds elapsed.
  while (GetSecs - stim1Onset) <= phaseCfg.comp_stim1
    % Wait <1 ms before checking the keyboard again to prevent
    % overload of the machine at elevated Priority():
    WaitSecs(0.0001);
  end
  ...
  WaitSecs(phaseCfg.comp_preStim2(1) + ((phaseCfg.comp_preStim2(2) - phaseCfg.comp_preStim2(1)).*rand(1,1)));
  
  % draw the stimulus
  Screen('DrawTexture', w, wiStim2Tex(i), [], stimImgRect);
  % and fixation on top of it
  Screen('TextSize', w, cfg.text.fixSize);
  DrawFormattedText(w,cfg.text.fixSymbol,'center','center',cfg.text.fixationColor, cfg.text.instructCharWidth);
  ...
  if cfg.text.printTrialInfo
    fprintf('Trial %d of %d: stim2 (%s): family %d (%s), species %d (%s), exemplar %d (%d). Same (1) or diff (0): %d.\n',i,length(wiStim2Tex),wiStim2(i).fileName,wiStim2(i).familyNum,wiStim2(i).familyStr,wiStim2(i).speciesNum,wiStim2(i).speciesStr,wiStim2(i).exemplarNum,wiStim2(i).exemplarName,wiStim2(i).same);
  end
  
  % while loop to show stimulus until subject response or until
  % "comp_stim2" seconds elapse.
  while (GetSecs - stim2Onset) <= phaseCfg.comp_stim2
    ...
    if ~phaseCfg.respDuringStim
      ...
      if keyIsDown && sum(keyCode) == 1
        ...
        while KbCheck(-1)
          ...
          % if (GetSecs - startRT) > phaseCfg.comp_response
          %   break
          % end
        end
        % if cfg.text.printTrialInfo
  ...
  if expParam.useNS
    ...
    % 'famn', family number
    % 'spcn', species number (corresponds to keyboard)
    % 'snum', stimulus number (order in comparison presentation, 1 or 2)
    ...
    [NSEventStatus, NSEventError] = et_NetStation('Event', 'FIXT', preStim1FixOn, .001,...
      'subn', expParam.subject, 'sess', sesName, 'phas', phaseName, 'pcou', int32(phaseCount),...
      'expt',phaseCfg.isExp,...
      'trln', int32(i), 'stmn', stim1Name, 'famn', fNum1, 'spcn', specNum1, 'snum', int32(wiStim1(i).compStimNum),...
      'sord', isSubord, 'trai', wiStim1(i).trained, 'same', wiStim1(i).same,...
      'rsps', resp, 'rspk', respKey, 'rspt', rt, 'corr', acc, 'keyp', keyIsDown); %#ok<NASGU,ASGLU>
    ...
    [NSEventStatus, NSEventError] = et_NetStation('Event', 'FIXT', preStim2FixOn, .001,...
      'subn', expParam.subject, 'sess', sesName, 'phas', phaseName, 'pcou', int32(phaseCount),...
      'expt',phaseCfg.isExp,...
      'trln', int32(i), 'stmn', stim2Name, 'famn', fNum2, 'spcn', specNum2, 'snum', int32(wiStim2(i).compStimNum),...
      'sord', isSubord, 'trai', wiStim2(i).trained, 'same', wiStim2(i).same,...
      'rsps', resp, 'rspk', respKey, 'rspt', rt, 'corr', acc, 'keyp', keyIsDown); %#ok<NASGU,ASGLU>
