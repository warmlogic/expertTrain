  ...
  if ismember(sesName,expParam.sesTypes)
    ...
    if ismember(phaseName,expParam.session.(sesName).phases)
      for phaseCount = 1:sum(ismember(expParam.session.(sesName).phases,phaseName))
        ...
        cfg.stim.(sesName).(phaseName)(phaseCount).instruct.expo(2).image = cfg.files.exposureRankRespKeyImg;
        cfg.stim.(sesName).(phaseName)(phaseCount).instruct.expo(2).imageScale = cfg.files.respKeyImgScale;
    ...
    if ismember(phaseName,expParam.session.(sesName).phases)
      for phaseCount = 1:sum(ismember(expParam.session.(sesName).phases,phaseName))
        ...
        [cfg.stim.(sesName).(phaseName)(phaseCount).instruct.cr(1).text] = et_processTextInstruct(...
          fullfile(cfg.files.instructDir,sprintf('%s_cr_1_practice_intro.txt',expParam.expName)),...
        ...
        [cfg.stim.(sesName).(phaseName)(phaseCount).instruct.cr(2).text] = et_processTextInstruct(...
          fullfile(cfg.files.instructDir,sprintf('%s_cr_2_practice_intro.txt',expParam.expName)),...
